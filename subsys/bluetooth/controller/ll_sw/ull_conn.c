...
uint8_t ull_fsu_update_eff_from_local(struct ll_conn *conn)
{
	...
	/* Capture the negotiated PHY(s) and spacing type into the effective
	 * parameters so they survive ull_fsu_update_eff() clearing the local
	 * request state; the RSP/notification encoders report eff.*.
	 */
