		      ...
		      bt_conn_tx_cb_t cb, void *user_data)
{
	...
	/* ATT_SENT_CB_AFTER_TX: att.c chan_send() takes an extra ref so the
	 * buf survives until the controller acks TX (chan_sent_cb releases it).
	 * In that case ref==2 is expected. All other callers (SMP, fixed-chan
	 * sent callbacks, etc.) pass ref==1.
	 */
#if defined(CONFIG_BT_ATT_SENT_CB_AFTER_TX)
	if (pdu->ref != 1 && pdu->ref != 2) {
#else
	if (pdu->ref != 1) {
#endif
		 ...
		 * this error.
		 */
		LOG_ERR("Expecting 1 ref (or 2 with ATT_SENT_CB), got %d", pdu->ref);
		return -EINVAL;
	}
