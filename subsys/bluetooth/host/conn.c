		    ...
		    size_t len, bt_conn_tx_cb_t cb, void *ud)
{
	...
	/* buf->ref is 1 if this was the only reference (e.g. buf was removed
	 * from the conn tx_queue). It is 2 if tx_data_pull kept it on the
	 * tx_queue for segmentation, or if ATT_SENT_CB_AFTER_TX holds an
	 * extra ref for the controller TX-complete callback.
	 */
